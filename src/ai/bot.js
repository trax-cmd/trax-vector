...
export function createBot(sim, team, opts = {}) {
  ...
  const cost = (b) => sim.price(b);   // the clock scales every price
  function answerTo(role, budget) {
    const cands = deck.filter((b) => cost(b) <= budget);
  ...
  const avgCost = () => deck.reduce((a, b) => a + cost(b), 0) / deck.length;
  ...
  function look(snap) {
    ...
    let sent = 0;
    const send = (b, from, goal) => { cmds.push({ op: 'deploy', team, tower: from.i, batt: b.id, goal }); energy -= cost(b); sent++; };
    ...
    if (sent < burst && energy - keep > cost(cheapest())) {
      ...
      if (open.length) {
        ...
        if (cost(b) <= energy - keep) { send(b, from, 1000 + w.i); claims.push([w.i, sim.tick]); }
      }
    }
    ...
    if (sent < burst && energy - keep > avg * waves) {
      ...
      for (let k = 0; k < waves && sent < burst; k++) {
        const b = k === 0 ? (deck.find((q) => q.role === 1 && cost(q) <= energy - keep) || answerTo(enemyTop, energy - keep)) : answerTo(enemyTop >= 0 ? enemyTop : r.int(6), energy - keep);
        if (!b || cost(b) > energy - keep) break;
        send(b, from, weak.i);
      }
    }
    return cmds;
